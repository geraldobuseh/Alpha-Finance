...
std::vector<Order> DollarCostAveraging::generateOrders(const MarketState& market,
    ...
    if (market.symbol().value() != "SPY" || std::chrono::weekday{day} != std::chrono::Monday)
        return {};
    ...
    for (const auto& event : history) {
        if (event.symbol() == market.symbol() && event.side() == OrderSide::Buy &&
    ...
    return {Order::create_market(next_id, market.symbol(), OrderSide::Buy, *quantity,
                                 market.timestamp())
                .value()};
}
